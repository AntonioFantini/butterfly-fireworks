...
%>
<a id="show_modules" href="">Menu</a>
<%
	}
%>

<%
	String allModulesClass = modules.hasModules() ? "main-navigation-left-with-all-modules"
	...
	<nobr>
		...
		<%
			for (Iterator it = modules.getTopModules().iterator(); it.hasNext();) {
				...
				if (module.getName().equals("SignIn"))
					continue;
				else if ("Role".equalsIgnoreCase(module.getLabel())
						|| "Utente".equalsIgnoreCase(module.getLabel())
						|| "First steps".equalsIgnoreCase(module.getLabel()))
					continue;
				String selected = module.getName().equals(
						request.getParameter("module")) ? "selected" : "";
		%>
		<a href="<%=modules.getModuleURI(request, module)%>?retainOrder=true"
			class="<%=selected%>"> <%=module.getLabel(request.getLocale())%>
		</a>
...
<span id="main_navigation_right"> <nobr>
		<span id="main_navigation_right_bridge1">&nbsp;&nbsp;&nbsp;</span><span
 ...
 %>
			...
			class="sign-in <%=selected%>"> <xava:message key="signin" />
		</a> <%
 	} else {
 %> <a href="<%=request.getContextPath()%>/naviox/signOut.jsp"
			class="sign-in"><xava:message key="signout" /> (<%=userName%>)</a> <%
 	}
 	}
 %>
		</span>
